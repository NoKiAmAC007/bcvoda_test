...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { title, slug, excerpt, content, fileUrl, image, publishedAt } = body;
  const updated = await prisma.report.update({
    ...
    data: {
      ...
      fileUrl: fileUrl !== undefined ? (fileUrl ? String(fileUrl) : null) : undefined,
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await prisma.report.update({ where: { id }, data: { ...(title && { title }), ...(slug && { slug: slugify(slug) }), excerpt: excerpt ?? undefined, content: content ?? undefined, fileUrl: fileUrl ?? undefined } });
  return NextResponse.redirect(new URL("/admin/reports", req.url), 303);
}
